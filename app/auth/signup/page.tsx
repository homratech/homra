...
import { ShieldCheck, User, Building2, Mail, Lock, Loader2, Phone, ArrowLeft } from 'lucide-react'
...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      await supabaseAuth.from('profiles').insert({ id: data.user.id, role, is_verified: false })
      handleRoute()
    }
  }
  ...
  const handleOAuth = async (provider: 'google' | 'apple') => {
    await supabaseAuth.auth.signInWithOAuth({
      provider,
      options: { redirectTo: window.location.origin + (role === 'landlord' ? '/dashboard/landlord' : '/dashboard/tenant') }
    })
  }
    ...
    <div className="min-h-screen grid md:grid-cols-2 bg-[#F5F5F7]">
      ...
      <div className="flex flex-col justify-center items-center p-6 md:p-10">
        <div className="w-full max-w-md bg-white p-8 rounded-2xl border border-[#E8EAED]" style={{ boxShadow: '0 4px 24px rgba(0,0,0,0.04)' }}>
            ...
            <>
              <form onSubmit={handleEmailSignup} className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <input type="password" placeholder="Create password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="w-full h-11 pl-10 pr-3 text-sm rounded-xl border border-[#E8EAED] focus:border-[#1A73E8] focus:outline-none focus:ring-1 focus:ring-[#1A73E8] transition-all" />
                </div>
          ...
          {authMode === 'phone-send' && (
            <>
              <button onClick={() => setAuthMode('email')} className="flex items-center gap-1 text-xs text-[#5F6368] hover:text-[#1A1A1A] mb-4"><ArrowLeft size={12} /> Back to email signup</button>
              <h1 className="text-xl font-bold text-[#1A1A1A] mb-1">Continue with Phone</h1>
              <p className="text-xs text-[#5F6368] mb-6">Enter your phone number. We'll send you a verification code.</p>
          ...
          <p className="text-center text-xs text-[#5F6368] mt-6">
            ...
            <Link href="/auth/login" className="text-[#1A73E8] font-semibold hover:underline">Log in</Link>
          </p>
        </div>
      </div>
    </div>
